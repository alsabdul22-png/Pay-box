...
const translations = {
    ...
    ar: {
        ...
        backOffice: 'مكتب العمليات',
        backOfficePassword: 'كلمة مرور مكتب العمليات',
        enterBackOfficePassword: 'يرجى إدخال كلمة المرور:',
        invalidPassword: 'كلمة مرور خاطئة!',
        ok: 'موافق',
        dashboard: 'لوحة التحكم',
        cashierReconciliation: 'مطابقة الصندوق',
        salesAnalysis: 'تحليل المبيعات',
        productManagement: 'إدارة المنتجات',
        inventoryManagement: 'إدارة المخزون',
        employeeManagement: 'إدارة الموظفين',
        todaySales: 'مبيعات اليوم',
        todayTransactions: 'المعاملات',
        avgTicket: 'متوسط الفاتورة',
        topProduct: 'المنتج الأفضل',
        salesByEmployee: 'المبيعات حسب الموظف',
        register: 'السجل',
        expectedCash: 'النقد المتوقع',
        actualCash: 'النقد المعدود',
        cardTransactions: 'معاملات البطاقة',
        difference: 'الفرق',
...
function loadDashboard() {
    ...
    document.getElementById('kpiTodaySales').textContent = '€' + todayTotalSales.toFixed(2);
    ...
    Object.keys(salesData).forEach(user => {
        chartHTML += `<tr><td>${user}</td><td>€${salesData[user].totalSales.toFixed(2)}</td><td>${salesData[user].customersCount}</td></tr>`;
    });
    chartHTML += '</table>';
        ...
        html += `
            <div class="article-item">
                <div class="article-item-header">
                    ...
                    <span class="article-sales">Verkäufe: ${sales}</span>
                </div>
                ...
                <div class="article-item-actions">
                    <button class="btn-edit-article" onclick="editArticle(${product.id})">✏️ Bearbeiten</button>
                    <button class="btn-delete-article" onclick="deleteArticleConfirm(${product.id})">🗑️ Löschen</button>
                </div>
            </div>
        `;
    });
...
function deleteArticleConfirm(productId) {
    const product = products.find(p => p.id === productId);
    if (product && confirm(`Artikel "${product.name}" wirklich löschen?`)) {
...
function calculateDifference() {
    ...
    if (difference > 0) {
        ...
        alertElement.innerHTML = `<div class="alert-success">✓ Plus: €${difference.toFixed(2)}</div>`;
    } else if (difference < 0) {
        diffElement.style.color = '#ff3b30';
        alertElement.innerHTML = `<div class="alert-error">✗ Minus: €${Math.abs(difference).toFixed(2)}</div>`;
    } else {
...
function generateSalesAnalysis() {
    ...
    html += '<table class="data-table"><tr><th>Produkt</th><th>Verkäufe</th><th>Umsatz</th></tr>';
    ...
    products.forEach(product => {
        ...
        if (count > 0) {
            html += `<tr><td>${product.name}</td><td>${count}</td><td>€${revenue.toFixed(2)}</td></tr>`;
        }
    });
    
    html += '</table><br><h4>Gesamt Produktumsatz: €' + totalProductSales.toFixed(2) + '</h4>';
...
function loadBackofficeProducts() {
    ...
    products.forEach(product => {
        ...
        html += `<div class="product-item-bo">
            <img src="${product.image || '🖼️'}" alt="${product.name}" style="width:50px;height:50px;border-radius:5px;">
            <div>
                <strong>${product.name}</strong><br>
                Preis: €${product.price.toFixed(2)} | Kategorie: ${product.category} | Verkäufe: ${sales}
            </div>
            <button class="btn-delete" onclick="deleteProduct(${product.id})">🗑️ Löschen</button>
        </div>`;
    });
...
function loadEmployeesView() {
    ...
    html += '<button class="btn-primary" onclick="startAllShifts()" style="margin-bottom:15px; background: #2ecc71;">▶ Alle Schichten starten</button>';
    html += '<button class="btn-primary" onclick="endAllShifts()" style="margin-bottom:15px; background: #ff3b30;">⏹ Alle Schichten beenden</button>';
    ...
    users.forEach(user => {
        ...
        const statusBadge = isActive ? '<span style="color:#2ecc71;">🟢 Aktiv</span>' : '<span style="color:#999;">⚪ Inaktiv</span>';
        ...
        html += `<tr>
            ...
            <td>€${sales.totalSales.toFixed(2)}</td>
...
function exportReportAsCSV() {
    ...
    csv += `Gesamtumsatz,€${report.totalSales.toFixed(2)}\n`;
    ...
    Object.keys(report.employees).forEach(user => {
        ...
        csv += `${user},€${emp.totalSales.toFixed(2)},${emp.customersCount}\n`;
    });
    
    csv += '\nZahlungsmethoden\n';
    csv += 'Methode,Betrag\n';
    csv += `Bargeld,€${report.paymentMethods.cash.toFixed(2)}\n`;
    csv += `Karte,€${report.paymentMethods.card.toFixed(2)}\n\n`;
    
    csv += 'Produkte\n';
    csv += 'Produkt,Verkäufe,Umsatz\n';
    report.products.forEach(product => {
        csv += `${product.name},${product.count},€${product.revenue.toFixed(2)}\n`;
    });
...
function generateHTMLReport() {
    ...
    <body>
        <div class="report-container">
            ...
            <div class="kpi">
                <div class="kpi-value">€${report.totalTransactions > 0 ? (report.totalSales / report.totalTransactions).toFixed(2) : '0.00'}</div>
                <div class="kpi-label">Ø Ticketgröße</div>
            </div>
            ...
            <table>
                ...
                ${Object.keys(report.employees).map(user => {
                    const emp = report.employees[user];
                    return `<tr><td>${user}</td><td>€${emp.totalSales.toFixed(2)}</td><td>${emp.customersCount}</td></tr>`;
                }).join('')}
            </table>
            ...
            <h2>Top Produkte</h2>
            <table>
                <tr><th>Produkt</th><th>Verkäufe</th><th>Umsatz</th></tr>
                ${report.products.sort((a, b) => b.revenue - a.revenue).slice(0, 10).map(product => 
                    `<tr><td>${product.name}</td><td>${product.count}</td><td>€${product.revenue.toFixed(2)}</td></tr>`
                ).join('')}
            </table>
